...
# AVIF removed from this set to prevent lossy-to-lossy conversion
...
def candidates(roots: list[Path]):
    ...
    for root in roots:
        ...
        for item in items:
            if not item.is_file() or item.suffix.lower() not in RASTER or any(part in EXCLUDED for part in item.parts):
                continue
...
def main():
    ...
    for source in candidates([Path(value) for value in args.paths]):
        ...
        try:
            ...
            # Prevents a single bad image from crashing the entire batch process
