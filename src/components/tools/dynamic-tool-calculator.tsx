...
type SimpleCalculatorConfig = {
  ...
  result: (values: { a: number; b: number; c: number }) => string
}
...
function formatPaceMinutes(value: number) {
...
const SIMPLE_CALCULATOR_CONFIGS: Record<string, SimpleCalculatorConfig> = {
  "tip-calculator": {
    labels: {
      a: "Bill Amount",
      b: "Tip Percentage",
    ...
    result: ({ a, b, c }) => {
      const people = Math.max(c, 1)
      const tip = a * (b / 100)
      const total = a + tip
      return `Tip: ${formatUsd(tip)} • Total: ${formatUsd(total)} • Per person: ${formatUsd(total / people)}`
    },
  },
  "discount-calculator": {
    labels: {
    ...
    result: ({ a, b, c }) => {
      ...
      const total = discounted * (1 + c / 100)
      const savings = a - discounted
      return `Savings: ${formatUsd(savings)} • Discounted price: ${formatUsd(discounted)} • Final total: ${formatUsd(total)}`
    },
  },
  "percentage-change-calculator": {
    ...
    result: ({ a, b }) => {
      if (a === 0) {
        return "Enter an original value other than 0 to calculate percentage change."
      }
      const difference = b - a
      const change = (difference / a) * 100
      return `Difference: ${difference.toFixed(2)} • Change: ${formatSignedPercent(change)}`
    },
  },
  "markup-calculator": {
    labels: {
    ...
    result: ({ a, b }) => {
      const sellingPrice = a * (1 + b / 100)
      return `Selling price: ${formatUsd(sellingPrice)} • Profit: ${formatUsd(sellingPrice - a)}`
    },
  },
  "margin-calculator": {
    labels: {
    ...
    result: ({ a, b }) => {
      if (a <= 0) {
        return "Enter a selling price greater than 0 to calculate margin."
      }
      const profit = a - b
      const margin = (profit / a) * 100
      return `Gross margin: ${formatSignedPercent(margin)} • Profit: ${formatUsd(profit)}`
    },
  },
  "fuel-cost-calculator": {
    ...
    result: ({ a, b, c }) => {
      if (b <= 0) {
        return "Enter an efficiency greater than 0 to estimate trip cost."
      }
      const litres = a / b
      return `Fuel needed: ${litres.toFixed(2)} L • Trip cost: ${formatUsd(litres * c)}`
    },
  },
  "electricity-cost-calculator": {
    labels: {
      a: "Appliance Power (Watts)",
      b: "Hours per Day",
      c: "Cost per kWh",
    },
    defaults: {
    ...
    result: ({ a, b, c }) => {
      const monthlyKwh = (a / 1000) * b * 30
      return `Monthly usage: ${monthlyKwh.toFixed(2)} kWh • Monthly cost: ${formatUsd(monthlyKwh * c)}`
    },
  },
  "pace-calculator": {
    labels: {
      a: "Distance (km)",
      b: "Total Time (minutes)",
    },
    defaults: {
      a: 10,
      b: 52,
    ...
    result: ({ a, b }) => {
      if (a <= 0 || b <= 0) {
        return "Enter a distance and time greater than 0 to calculate pace."
      }
      const pace = b / a
      const speed = a / (b / 60)
      return `Pace: ${formatPaceMinutes(pace)} / km • Average speed: ${speed.toFixed(2)} km/h`
    },
  },
  "compound-interest-calculator": {
    ...
    result: ({ a, b, c }) => {
      const futureValue = a * (1 + b / 100) ** c
      return `Future value: ${formatUsd(futureValue)} • Interest earned: ${formatUsd(futureValue - a)}`
    },
  },
  "speed-distance-time-calculator": {
    ...
    result: ({ a, b, c }) => {
      const totalHours = Math.max(b, 0) + Math.max(c, 0) / 60
      return `Distance traveled: ${(a * totalHours).toFixed(2)} km in ${totalHours.toFixed(2)} hours`
    },
  },
}
...
function CalculatorTools({ toolId }: { toolId: string }) {
  ...
  const simpleCalculatorConfig = SIMPLE_CALCULATOR_CONFIGS[toolId]

  useEffect(() => {
    if (!simpleCalculatorConfig) {
      return
    }

    setA(simpleCalculatorConfig.defaults.a)
    setB(simpleCalculatorConfig.defaults.b)
    setC(simpleCalculatorConfig.defaults.c)
  ...
  const result = useMemo(() => {
    if (simpleCalculatorConfig) {
      return simpleCalculatorConfig.result({ a, b, c })
    }
      ...
      <ToolCard title="Inputs">
        <div className="space-y-4">
          {toolId === "time-zone-converter" || toolId === "age-calculator" || toolId === "date-difference-calculator" ? (
            ...
            <>
              <div>
                <Label>{simpleCalculatorConfig.labels.a}</Label>
                <Input type="number" value={a} onChange={(event) => setA(Number(event.target.value) || 0)} className="mt-2" />
              </div>
              {simpleCalculatorConfig.labels.b ? (
                <div>
                  <Label>{simpleCalculatorConfig.labels.b}</Label>
                  <Input type="number" value={b} onChange={(event) => setB(Number(event.target.value) || 0)} className="mt-2" />
                </div>
              ) : null}
              {simpleCalculatorConfig.labels.c ? (
                <div>
                  <Label>{simpleCalculatorConfig.labels.c}</Label>
